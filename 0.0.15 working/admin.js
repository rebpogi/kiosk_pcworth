...
// Show modal if ?success=1 is in URL
...
$(document).ready(function () {
  ...
  // Load default product list on page load inside #default-table
  $('#default-table').load('load_default_table.php');
  ...
  $('#search').on('keyup', function () {
    ...
    if (query === '') {
      // If search input is empty, hide search results and show default table
      $('#result').html('').hide();
      $('#default-table').show();
      return; // stop here, no AJAX search request sent
    }

    // If there is a query, hide default table and show search results container
    $('#default-table').hide();
  ...
  if (form && form.id === "Update_product_form") {
    ...
    fetch("update_product.php", {
      ...
      .then(result => {
        ...
        document.getElementById("search-btn").click(); // <-- trigger refresh
      })
