...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $status = isset($_POST['checkbox']) ? 'Hidden' : 'Available';
    ...
    $stmt = $conn->prepare("INSERT INTO products (
        product_display_name, price, quantity, UID, category, manufacturer,
        product_specifications, product_description, status, immage, warranty_duration, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())");

    $stmt->bind_param("sdissssssss",
        $name, $price, $quantity, $uid, $category, $manufacturer,
        $specs, $description, $status, $imagePath, $warranty
    );
